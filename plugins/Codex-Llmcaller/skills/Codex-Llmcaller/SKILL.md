...
本节只适用于已经实际调用 Codex-Llmcaller 外部模型工具的回复。没有调用插件时，不要在普通 Codex 回答、其他项目回答或未使用外部模型的结论中套用这些字段。

为控制 Codex 上下文消耗，默认使用短格式，不展开完整 JSON：

- `提交给插件的问题`：仅在审查、核对、评价或参与审计场景中展示，说明实际交给外部模型的问题。
- `插件结论`：展示外部模型的 `opinion` 和 `conclusion`；没有额外建议时写 `无额外建议。`。
- `Codex 处理`：如插件给出建议，逐条说明 `采纳`、`不采纳` 或 `延后`；没有建议时写 `无需调整。`。
- `插件信息`：保留插件返回的短元信息，至少包括 provider/model、profile、token 数、fallback 状态；审查循环中还要包括当前轮次。优先直接保留插件 footer，避免 Codex 重新组织成长文本。

如果需要多轮审查，后续调用应传入 `reviewIteration`，例如第 2 轮传 `reviewIteration: 2`。最终回答只展示必要轮次信息，不展开每轮完整原文，除非用户明确要求追溯。
