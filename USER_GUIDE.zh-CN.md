...
适合普通问答、图片/音频/视频/文档输入、需要 Google Search grounding 的联网场景：
...
对应使用 `deepseek-pro` profile。DeepSeek 不支持 Gemini Google Search grounding 和多模态输入；需要联网、图片、音频、视频或文档理解时应使用 Gemini。
...
这类请求应使用 `routingMode: "auto"`。自动路由会在可用时优先用 DeepSeek 做上文核对；遇到今天、最新、天气、新闻、价格、搜索、联网、实时等信息时切到 Gemini grounded；遇到图片、音频、视频或文档输入时使用 Gemini。
...
新调用优先使用 `mediaInputs`。旧的 `imageInputs` 会继续兼容，但只适合图片；`mediaInputs` 可以传文本补充说明、图片、音频、视频、PDF/文档、本地路径、URL，或已经通过 Gemini File API 预上传得到的 `fileUri`。
...
已经预上传到 Gemini File API 的大文件使用 `fileUri`：

```json
{
  "profileName": "gemini-default",
  "prompt": "请阅读这份 PDF，列出主要结论。",
  "mediaInputs": [
    {
      "type": "document",
      "fileUri": "files/your-uploaded-file",
...
当前阶段不会自动替用户上传大文件；如果设置 `transfer: "file_uri"` 但没有提供 `fileUri`，插件会返回明确错误。后续节点会加入自动 File API 上传。
...
- 带图片、音频、视频或文档输入时，优先使用支持 `mediaInputs` 的 Gemini profile；旧图片输入仍兼容 `imageInputs`。
...
review JSON 不能只返回非阻塞建议，必须给出可读自然语言审计结论。插件要求所有模型使用四类核心审计意见：

- `无保留意见`：未发现实质阻塞问题，可以采纳；仍可附带改进建议。
- `保留意见`：整体可用，但存在必须说明或修复的实质保留事项。
- `否定意见`：存在重大错误、风险或不适合采纳的问题。
- `无法表示意见`：上下文、证据、测试或材料不足，无法形成可靠判断。

`outputJson` 会包含或补齐以下字段：

```json
{
  "opinion": "无保留意见",
  "conclusion": "审计结论：无保留意见。未发现阻塞问题。",
  "basis": "形成该意见的主要依据。",
  "recommendation": "无额外建议。"
}
```

即使模型只返回 `suggested_correction`，插件也会补齐 `opinion`、`conclusion`、`basis` 和 `recommendation`，避免最终输出只有建议、没有明确结论。
...
- `full`: 直接返回完整模型文本。
- `json`: review 场景默认模式，要求模型返回紧凑 JSON，并在结构化结果中提供 `outputJson`；审计结论字段包含 `opinion`、`conclusion`、`basis`、`recommendation`。
- `summary`: 要求 review 场景返回简短自然语言摘要，并以 `无保留意见`、`保留意见`、`否定意见` 或 `无法表示意见` 开头。
...
可用 `provider_capabilities` 查看路由能力表，包括 Gemini、DeepSeek、Anthropic 和 OpenAI-compatible provider 是否支持 JSON、图片、音频、视频、文档、联网 grounding、thinking 等能力。
